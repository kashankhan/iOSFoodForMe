...
class FFMRecipesParser: FFMBaseParser {
    ...
    private func parseRecipe(response: AnyObject, context: DataContext) -> Recipe? {
        ...
        let recipe = context.recipes.firstOrCreated(whereAttribute: "recipeId", isEqualTo: recipeId)
    ...
    private func parseNutritionInfo(response: AnyObject?, context: DataContext, recipe: Recipe) -> NutritionInfo? {
        let nutritionInfo = context.nutritions.firstOrCreated(whereAttribute: "identifier", isEqualTo: recipe.recipeId)
    ...
    private func parseIngredient(response: AnyObject, context: DataContext, recipe: Recipe) -> Ingredient {
        ...
        let ingredient = context.ingredients.firstOrCreated(whereAttribute: "ingredientId", isEqualTo: ingredientId)
        ingredient.displayIndex = response["displayIndex"] as! Int
        ingredient.ingredientId = ingredientId as String
    ...
    private func parseRecommendedRecipe(response: AnyObject, context: DataContext) -> RecommendedRecipe {
        ...
        let recommendedRecipe = context.recommendedRecipes.firstOrCreated(whereAttribute: "identifier", isEqualTo: recipe.recipeId)
    ...
    private func getRecommendedRecipeExplaintation(preferCookingTime: Int, favoriteIngredientsInRepcie: String, favoriteIngredients: [String], recipe : Recipe) -> String {
        var explaintion: String = NSString(format: NSLS.calaroiesAndExcersiseInfomation, randomInt(500, max: 5000), randomInt(10, max: 60)) as String
        var isUserContextSet = false
        if !favoriteIngredients.isEmpty {
            explaintion = (explaintion as String) + " " + (NSString(format: NSLS.ingredientsExplaination, favoriteIngredientsInRepcie) as String)
            
            isUserContextSet = true
        }
        if preferCookingTime > 0 {
           explaintion = (explaintion as String) + " " +  (NSString(format: NSLS.preferCookingTimeExplaination, preferCookingTime) as String)
            isUserContextSet = (isUserContextSet == true) ? isUserContextSet : false
        }
        if isUserContextSet == false {
            explaintion = (explaintion as String) + " " + (NSString(format: NSLS.popularRecipeExplaination, recipe.category) as String)
        }
        
        return explaintion as String
    }
    ...
    private func parseCourse(response: AnyObject, context: DataContext) -> Course {
       let course = context.courses.firstOrCreated(whereAttribute: "name", isEqualTo: response)
